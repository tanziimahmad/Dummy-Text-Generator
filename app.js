...
let paragraphLength = document.querySelector('#amount');

let buttonPress = document.querySelector('#btn');

let shownResult = document.querySelector('#result');

buttonPress.addEventListener('click', function (rashik) {

  rashik.preventDefault();
  let length = Number(paragraphLength.value);
  console.log(typeof length, length);
  if (length <= 9) {
    
  let dummyTextArray = dummyText.slice(0, length);
  
  // console.log(dummyTextArray);

  let final = dummyTextArray.map(function (i) {
    return `<p>${i}</p>`
  }).join('');

  shownResult.innerHTML = final;

    console.log(final);
    
    } else {
    alert("Please put number between 1-9");

    // let pick = math.floor(Math.random() * 9);
    
    shownResult.innerHTML=dummyText[0];
  }
})